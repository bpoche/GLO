...
def rec_imu_async(imu_com_port,
    ...
    try:
        ...
        print('imu baudrate =',ez.sensor.baudrate)
        print('imu frequency =',ez.sensor.read_async_data_output_frequency())
    except:
    ...
    while loop_trigger.value==1:
        ...
        try:
            ...
            mag.append(ez.current_data.magnetic)
    ...
    for i in range(len(ypr)):
        try:
            ...
            mag_x.append(mag[i].x)
            mag_y.append(mag[i].y)
            mag_z.append(mag[i].z)            
        except:
            yaw.append(np.nan)
            pitch.append(np.nan)
            roll.append(np.nan)
